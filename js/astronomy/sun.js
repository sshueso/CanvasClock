...
/*
obs.properties:
name ='', year = 2011, month = 2, day = 3, hours = 5, minutes = 46, seconds = 57, tz(timezone) = 0, latitude = 51.47722, longitude = 0
*/
	...
	sunObj.time = timestring(obscopy,false);
	sunObj.altitude = Math.round(altaz[0]);
	sunObj.azimuth = Math.round(altaz[1]);
	sunObj.sunrise = riseset[0];
	sunObj.sunset = riseset[1];
	
	/*var sunr = riseset[0].split(':');
	var suns = riseset[1].split(':');
	sunObj.midday = ((suns[0] - sunr[0]) /2) + Math.round(sunr[0]);
	*/
